"""
MCMO Baseline Adapter - Pymoo Version

Uses pymoo for NSGA-II (not correlation).
Full multi-objective feature selection.
...
# Try to import MCMO with pymoo
try:
    # Use absolute imports for Colab compatibility
    from mcmo.MCMO_pymoo import MCMO_Pymoo, RiverTreeWrapper, RiverADWINWrapper
    MCMO_AVAILABLE = True
except ImportError as e:
    MCMO_AVAILABLE = False
    IMPORT_ERROR = str(e)
    warnings.warn(f"MCMO dependencies not available: {IMPORT_ERROR}\nPlease install: pymoo river scipy scikit-learn")


class MCMOAdapter:
    """MCMO Adapter - Pymoo version with real NSGA-II."""
    ...
    def __init__(
        ...
        # Initialize MCMO with pymoo
        self.mcmo = MCMO_Pymoo(
    ...
    def get_info(self) -> dict:
        return {
            ...
            'nsga_pop_size': self.mcmo.nsga_pop_size,
            'nsga_n_gen': self.mcmo.nsga_n_gen
        }


class MCMOEvaluator:
    """Evaluator wrapper for MCMO Pymoo."""
...
if __name__ == "__main__":
    print("MCMO Baseline Adapter (Pymoo - Real NSGA-II)")
    print("=" * 50)

    if not MCMO_AVAILABLE:
        print(f"ERROR: {IMPORT_ERROR}")
        print("\nPlease install dependencies:")
        print("  pip install pymoo river scipy scikit-learn")
    else:
        print("MCMO Pymoo dependencies available ✓")
        print("\nTo use this adapter:")
        print("  from mcmo.baseline_mcmo_pymoo import MCMOAdapter, MCMOEvaluator")
